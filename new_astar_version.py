...
pygame.display.set_caption("Red Ball Chasing Blue Ball")
...
while running:
    ...
    while True:  # Inner loop for a single chase session
        ...
        blue_ball_pos[0] += random.randint(-3, 3)
        blue_ball_pos[1] += random.randint(-3, 3)

        # Ensure blue ball stays within the frame
        blue_ball_pos[0] = max(BALL_RADIUS, min(int(blue_ball_pos[0]), SCREEN_WIDTH - BALL_RADIUS))
        blue_ball_pos[1] = max(BALL_RADIUS, min(int(blue_ball_pos[1]), SCREEN_HEIGHT - BALL_RADIUS))
